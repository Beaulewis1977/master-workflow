...
analyze_project() {
    ...
    echo -e "${CYAN}🧠 Analyzing project complexity...${NC}"
    ...
    echo -e "\n${BOLD}Project Analysis Summary:${NC}"
    echo -e "  ${CYAN}Complexity Score:${NC} ${score}/100"
    echo -e "  ${CYAN}Project Stage:${NC} ${stage}"
    echo -e "  ${CYAN}Analysis Confidence:${NC} ${confidence}%"

    # Show detected features if available
    if [ "$(echo "$analysis" | jq -r '.factors.features')" != "null" ]; then
        echo -e "\n${BOLD}Detected Features:${NC}"
        echo "$analysis" | jq -r '.factors.features.detected | to_entries[] | select(.value == true) | "  ✓ \(.key)"'
    fi
...
show_interactive_menu() {
    ...
    # Display Simple Swarm option
    local swarm_match=$(node -e "
        const rec = $recommendation;
        const alt = rec.alternatives.find(a => a.name === 'Simple Swarm');
        console.log(alt ? alt.matchScore : rec.matchScore);
    ")
    ...
    local hive_match=$(node -e "
        const rec = $recommendation;
        const alt = rec.alternatives.find(a => a.name === 'Hive-Mind');
        console.log(alt ? alt.matchScore : rec.matchScore);
    ")
    ...
    local sparc_match=$(node -e "
        const rec = $recommendation;
        const alt = rec.alternatives.find(a => a.name === 'Hive-Mind + SPARC');
        console.log(alt ? alt.matchScore : rec.matchScore);
    ")
